...
from API.hyperframes_api import router as hyperframes_router
app.include_router(hyperframes_router)
...
QUEUE = []
QUEUE_LOCK = Lock()
LOCAL_COMFY_TASK_STATUS: Dict[str, Dict[str, Any]] = {}
LOCAL_COMFY_TASK_STATUS_LOCK = Lock()
LOCAL_COMFY_CLIENT_TASK_MAP: Dict[str, str] = {}
LOCAL_COMFY_CLIENT_TASK_MAP_LOCK = Lock()
HISTORY_LOCK = Lock()
...
def read_api_env_values():
    ...
    try:
        ...
        print(f"读取 API/.env 失败: {e}")
    return values

def write_api_env_values(updates):
...
AUTODL_OPENAI_BASE = os.getenv("AUTODL_OPENAI_BASE", "https://www.autodl.art/api/v1").rstrip("/")
AUTODL_GEMINI_BASE = os.getenv("AUTODL_GEMINI_BASE", "https://www.autodl.art/api/v1/gemini").rstrip("/")
AUTODL_CHAT_MODELS = [m.strip() for m in os.getenv("AUTODL_CHAT_MODELS", "qwen3.6-plus,Qwen3.5-397B-A17B,Kimi-K2.5,gpt-5.4-mini,gpt-5.4-nano,gpt-5.4,gpt-5.5,gemini-3.1-pro-preview").split(",") if m.strip()]
...
class ApiKeySettingsRequest(BaseModel):
    ...
    autodl_api_key: Optional[str] = None
...
def normalize_custom_base_url(value: str):
...
def custom_api_headers(api_key: str, json_body=True):
    key = (api_key or "").strip()
    if not key:
        raise HTTPException(status_code=400, detail="Please enter API Key")
    headers = {"Accept": "application/json", "Authorization": f"Bearer {key}"}
    if json_body:
        headers["Content-Type"] = "application/json"
    return headers

def resolve_chat_provider(provider: str, model: str, ms_model: str, base_url: str = "", api_key: str = ""):
    custom_base = normalize_custom_base_url(base_url)
    if custom_base or (api_key or "").strip():
        if not custom_base:
            raise HTTPException(status_code=400, detail="Please enter API URL")
        mdl = (model or ms_model or "").strip()
        if not mdl:
            raise HTTPException(status_code=400, detail="Please select a model ID")
        return custom_base, custom_api_headers(api_key), mdl
    ...
    if provider == "autodl":
        if not AUTODL_API_KEY:
            raise HTTPException(status_code=400, detail="未配置 AUTODL_API_KEY，请在画布 API Key 设置中填写。")
        base = AUTODL_OPENAI_BASE
        hdrs = {"Authorization": f"Bearer {AUTODL_API_KEY}", "Content-Type": "application/json", "Accept": "application/json"}
        mdl = selected_model(model, AUTODL_CHAT_MODELS[0] if AUTODL_CHAT_MODELS else "qwen3.6-plus")
        return base, hdrs, mdl
    base = AI_BASE_URL + "/v1"
    hdrs = api_headers()
...
def _set_local_comfy_task_status(task_id: str, patch: Dict[str, Any]):
    with LOCAL_COMFY_TASK_STATUS_LOCK:
        current = LOCAL_COMFY_TASK_STATUS.get(task_id, {})
        current.update(patch)
        current["updated_at"] = time.time()
        LOCAL_COMFY_TASK_STATUS[task_id] = current

def _get_queue_position(task_id: str):
    with QUEUE_LOCK:
        total = len(QUEUE)
        position = 0
...
def _emit_local_comfy_status(client_id: str, payload: Dict[str, Any]):
    if not client_id or not GLOBAL_LOOP:
        return
    try:
        asyncio.run_coroutine_threadsafe(
            manager.send_personal_message({"type": "local_comfy_status", **payload}, client_id),
            GLOBAL_LOOP
        )
    except Exception as e:
        print(f"emit local comfy status error: {e}")

async def wait_for_image_task(client, task_id, base_url=None, headers=None):
    deadline = time.monotonic() + AI_REQUEST_TIMEOUT
    last_payload = {}
    while time.monotonic() < deadline:
        response = await client.get(f"{(base_url or AI_BASE_URL + '/v1').rstrip('/')}/images/tasks/{task_id}", headers=headers or api_headers())
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, base_url="", api_key=""):
    refs = [ref for ref in (reference_images or []) if ref.get("url")]
    custom_base = normalize_custom_base_url(base_url)
    image_base = custom_base or f"{AI_BASE_URL}/v1"
    json_headers = custom_api_headers(api_key) if custom_base else api_headers()
    form_headers = custom_api_headers(api_key, json_body=False) if custom_base else api_headers(json_body=False)
    async with httpx.AsyncClient(timeout=AI_REQUEST_TIMEOUT) as client:
        if refs:
            files = []
            opened = []
            try:
                ...
                response = await client.post(f"{image_base.rstrip('/')}/images/edits", headers=form_headers, data=data, files=files)
            finally:
                for fh in opened:
                    fh.close()
        else:
            response = await client.post(
                f"{image_base.rstrip('/')}/images/generations",
        ...
        try:
            ...
            if not task_id:
                raise
        task_result = await wait_for_image_task(client, task_id, image_base, json_headers)
...
async def ai_config():
    ...
    return {
        ...
        "autodl_chat_models": AUTODL_CHAT_MODELS,
        "has_autodl_key": bool(AUTODL_API_KEY),
    }

@app.get("/api/settings/api-keys")
async def get_api_key_settings():
    env_values = read_api_env_values()
    comfly_key = env_values.get("COMFLY_API_KEY", AI_API_KEY)
    modelscope_key = env_values.get("MODELSCOPE_API_KEY", MODELSCOPE_API_KEY)
    autodl_key = env_values.get("AUTODL_API_KEY", AUTODL_API_KEY)
    return {
        "has_comfly_api_key": bool(comfly_key),
        "comfly_api_key": comfly_key,
        "comfly_api_key_masked": mask_api_key(comfly_key),
        "has_modelscope_api_key": bool(modelscope_key),
        "modelscope_api_key": modelscope_key,
        "modelscope_api_key_masked": mask_api_key(modelscope_key),
        "has_autodl_api_key": bool(autodl_key),
        "autodl_api_key": autodl_key,
        "autodl_api_key_masked": mask_api_key(autodl_key),
    }

@app.post("/api/settings/api-keys")
async def save_api_key_settings(payload: ApiKeySettingsRequest):
    global AI_API_KEY, MODELSCOPE_API_KEY, AUTODL_API_KEY
    updates = {}
    if payload.comfly_api_key is not None:
        updates["COMFLY_API_KEY"] = payload.comfly_api_key.strip()
    if payload.modelscope_api_key is not None:
        updates["MODELSCOPE_API_KEY"] = payload.modelscope_api_key.strip()
    if payload.autodl_api_key is not None:
        updates["AUTODL_API_KEY"] = payload.autodl_api_key.strip()
    if updates:
        write_api_env_values(updates)
    env_values = read_api_env_values()
    AI_API_KEY = env_values.get("COMFLY_API_KEY", "")
    MODELSCOPE_API_KEY = env_values.get("MODELSCOPE_API_KEY", "")
    AUTODL_API_KEY = env_values.get("AUTODL_API_KEY", "")
    os.environ["COMFLY_API_KEY"] = AI_API_KEY
    os.environ["MODELSCOPE_API_KEY"] = MODELSCOPE_API_KEY
    os.environ["AUTODL_API_KEY"] = AUTODL_API_KEY
    return {
        "success": True,
        "has_comfly_api_key": bool(AI_API_KEY),
        "comfly_api_key": AI_API_KEY,
        "comfly_api_key_masked": mask_api_key(AI_API_KEY),
        "has_modelscope_api_key": bool(MODELSCOPE_API_KEY),
        "modelscope_api_key": MODELSCOPE_API_KEY,
        "modelscope_api_key_masked": mask_api_key(MODELSCOPE_API_KEY),
        "has_autodl_api_key": bool(AUTODL_API_KEY),
        "autodl_api_key": AUTODL_API_KEY,
        "autodl_api_key_masked": mask_api_key(AUTODL_API_KEY),
    }

@app.get("/api/models")
async def ai_models():
    return {"chat_models": CHAT_MODELS, "image_models": IMAGE_MODELS, "autodl_chat_models": AUTODL_CHAT_MODELS}
...
async def online_image(payload: OnlineImageRequest):
    custom_image_api = bool((payload.base_url or "").strip() or (payload.api_key or "").strip())
    model = (payload.model or "").strip() if custom_image_api else selected_model(payload.model, IMAGE_MODEL)
    if custom_image_api and not model:
        raise HTTPException(status_code=400, detail="Please select an image model ID")
    refs = [ref.dict() for ref in payload.reference_images if ref.url]
    try:
        image_data, raw = await generate_ai_image(payload.prompt, payload.size, payload.quality, model, refs, payload.base_url, payload.api_key)
...
async def canvas_llm(payload: CanvasLLMRequest):
    chat_base, chat_hdrs, model = resolve_chat_provider(payload.provider, payload.model, payload.ms_model, payload.base_url, payload.api_key)
...
async def get_generate_status(task_id: str = "", client_task_id: str = ""):
    resolved_task_id = str(task_id or "").strip()
    resolved_client_task_id = str(client_task_id or "").strip()
    if not resolved_task_id and resolved_client_task_id:
        with LOCAL_COMFY_CLIENT_TASK_MAP_LOCK:
            resolved_task_id = LOCAL_COMFY_CLIENT_TASK_MAP.get(resolved_client_task_id, "")
    if not resolved_task_id:
        if resolved_client_task_id:
            return {"success": False, "task_id": "", "client_task_id": resolved_client_task_id, "message": "task not found"}
        return {"success": False, "task_id": "", "message": "task_id or client_task_id is required"}
    with LOCAL_COMFY_TASK_STATUS_LOCK:
        data = LOCAL_COMFY_TASK_STATUS.get(resolved_task_id)
    if not data:
        return {"success": False, "task_id": resolved_task_id, "message": "task not found"}
    return {"success": True, "task_id": resolved_task_id, **data}
...
def generate(req: GenerateRequest):
    ...
    prompt_id = ""
    ws_conn = None
    ws_client_id = f"wuli_local_{uuid.uuid4().hex}"
    latest_progress_value = 0
    latest_progress_max = 0
    latest_node = ""
    with QUEUE_LOCK:
    ...
    client_task_id = str(req.client_task_id or "").strip()
    if client_task_id:
        with LOCAL_COMFY_CLIENT_TASK_MAP_LOCK:
            LOCAL_COMFY_CLIENT_TASK_MAP[client_task_id] = task_id_str

    queue_total, queue_position = _get_queue_position(task_id_str)
    _set_local_comfy_task_status(task_id_str, {
        "status": "queued",
        "task_id": task_id_str,
    ...
    _emit_local_comfy_status(req.client_id, {
        "task_id": task_id_str,
        "client_task_id": client_task_id,
        "status": "queued",
        "queue_total": queue_total,
    ...
    try:
        ...
        try:
            ws_conn = create_connection(f"ws://{target_backend}/ws?clientId={ws_client_id}", timeout=1)
            ws_conn.settimeout(0.15)
        except Exception as ws_error:
            ws_conn = None
            print(f"Local Comfy ws attach failed: {ws_error}")
        ...
        p = {"prompt": workflow, "client_id": ws_client_id}
        ...
        for _ in range(300):
                ...
                while True:
                    try:
                        ...
                        if msg_type == "progress" and msg_data.get("prompt_id") == prompt_id:
                            ...
                            node_name = msg_data.get("node")
                            if isinstance(value, (int, float)):
                                latest_progress_value = int(value)
                            if isinstance(max_value, (int, float)) and max_value > 0:
                                latest_progress_max = int(max_value)
                            if node_name is not None:
                                latest_node = str(node_name)
            ...
            progress_percent = 0
            if latest_progress_max > 0:
                progress_percent = int(max(0, min(100, round((latest_progress_value / latest_progress_max) * 100))))
            ...
            _set_local_comfy_task_status(task_id_str, {
                ...
                "queue_total": queue_total,
                "queue_position": queue_position,
                "progress_value": latest_progress_value,
                "progress_max": latest_progress_max,
        ...
        _set_local_comfy_task_status(task_id_str, {
            ...
            "queue_total": 0,
            "queue_position": 0,
            "progress_value": latest_progress_max if latest_progress_max > 0 else latest_progress_value,
            "progress_max": latest_progress_max if latest_progress_max > 0 else latest_progress_value,
            "progress_percent": 100,
            "current_node": "completed",
            "result_count": len(local_urls)
        })
        _emit_local_comfy_status(req.client_id, {
            "task_id": task_id_str,
            "client_task_id": client_task_id,
        ...
        return {"images": [], "error": str(e), "task_id": task_id_str}
    finally:
        if ws_conn:
            try:
                ws_conn.close()
